...
def loop(
    ...
    **kwargs,
    ...
    histograms = HISTOGRAMS.get()
    if len(histograms) == 0:
        get_logger().warning("No histograms available for evaluation.")
        return
    _t0 = time.time()
    positions = model.evaluate(histograms)
    _t1 = time.time()
    get_logger().info(
        f"Model evaluation took {_t1 - _t0:.4f} seconds for {len(histograms)} histograms."
    )
