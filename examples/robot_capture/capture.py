...
def setup(manager: Manager):
    ...
    spad = SPADSensor.create_from_registry(SPAD_NAME, port=SPAD_PORT, spad_id=6)
    if not spad.is_okay:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--spad-port", help="Port that the SPAD sensor is on.", required=False
    )
    parser.add_argument(
        "--gantry-port", help="Port that the gantry is on.", required=False
    )
